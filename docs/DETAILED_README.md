...
- `dashboard/app.py` — Streamlit dashboard with cohort-first UX:
  - **Left sidebar**: Multi-select Risk Strata and Exposure Status filters; Minimum Threshold slider; optional NLQ-based cohort restriction (sidebar queries `rwe_cohort`, `ckd_risk` tables via Claude text-to-SQL)
  ...
  - **Right panel**: Categorical differences boxplot (selectable value and category; statistical group comparison)
