...
|---|---|
| Full demo pipeline (synthetic/default) | `make run` or `make run-synthetic` |
| Optional Diabetes130 pipeline | `make run-diabetes130` or `ETL_SOURCE=diabetes130 .venv/bin/python -m etl.pipeline` |
...
### 3e. Source selection contract

`etl/pipeline.py` reads `ETL_SOURCE`:
- `ETL_SOURCE=synthetic` (default): load `data/raw/synthea/csv/` if present, otherwise generate synthetic/demo.
- `ETL_SOURCE=diabetes130`: map `data/raw/diabetes130/diabetic_data.csv` into canonical tables.
...
The pipeline will detect CSV files in `data/raw/synthea/csv/` and load them instead of generating synthetic/demo rows. DuckDB is fully rebuilt from scratch on every run.

For Diabetes130 explicitly:
